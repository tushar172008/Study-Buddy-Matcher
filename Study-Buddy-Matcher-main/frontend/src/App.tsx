...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedProfile) {
      try {
        ...
        setUserProfile({
          ...parsedProfile,
          university: parsedProfile.university || userProfile.university || parsedProfile.email?.split('@')[1] || ''
        });
  ...
  const universityKey = (profile: { university?: string; email: string }) =>
    (profile.university || profile.email.split('@')[1] || '').trim().toLowerCase();

  // Discovery is limited to the same university; courses remain editable preferences.
  const baseDiscoverable = studentsList.filter(
    s => !blockedIds.includes(s.id) && 
         ...
         universityKey(s) === universityKey(userProfile)
  );
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f8f7fb] text-slate-900 font-sans" id="app-root-container">
      {/* Top Banner Navigation (Academic design, pure white background, crisp borders) */}
      <header className="border-b border-slate-100/80 bg-white/90 backdrop-blur-md sticky top-0 z-40" id="app-header">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between" id="header-inner">
      ...
      {/* Main Content Arena */}
      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5 sm:py-8" id="app-main-content">
        
        {/* Navigation Tabs (Horizontal slide feel, clear contrast) */}
        <div className="flex border-b border-slate-100 mb-5 sm:mb-8 overflow-x-auto scrollbar-none gap-1 sm:gap-2" id="nav-tabs">
          <button
            type="button"
            id="tab-discover"
            onClick={() => setActiveTab('discover')}
            className={`py-3 px-3 sm:px-4 text-xs font-bold uppercase tracking-wider border-b-2 transition-all flex items-center gap-2 shrink-0 ${
              activeTab === 'discover'
        ...
        <div className="mt-4" id="tab-views-root">
          {activeTab === 'discover' && (
            <div className="space-y-8" id="view-discover">
              <motion.section
                initial={{ opacity: 0, y: 26 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.25 }}
                transition={{ duration: 0.6, ease: 'easeOut' }}
                className="relative isolate overflow-hidden rounded-[2rem] bg-[#28203c] px-6 py-8 text-white shadow-xl shadow-violet-950/10 sm:px-10 sm:py-12 lg:min-h-[310px]"
                id="discover-hero"
              >
                <div className="absolute -right-16 -top-24 -z-10 h-72 w-72 rounded-full bg-fuchsia-500/90 sm:h-96 sm:w-96" />
                <div className="absolute -bottom-36 left-1/3 -z-10 h-72 w-72 rounded-full bg-violet-500/20" />
                <div className="absolute right-8 top-8 hidden h-40 w-40 rotate-6 rounded-[2rem] border border-white/15 bg-white/10 p-4 lg:block">
                  <div className="h-full rounded-2xl bg-[url('https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=500&q=80')] bg-cover bg-center opacity-80" />
                </div>
                <div className="relative max-w-xl">
                  <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.24em] text-fuchsia-200">Your study community</p>
                  <h2 className="max-w-lg text-3xl font-semibold leading-tight tracking-tight sm:text-5xl">Find your people. Learn your way.</h2>
                  <p className="mt-4 max-w-md text-sm leading-6 text-violet-100">Discover compatible classmates, build focused study circles, and turn shared courses into shared momentum.</p>
                  <div className="mt-7 flex flex-wrap gap-2">
                    <span className="rounded-full bg-white px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-[#28203c]">{discoverableStudents.length} matches waiting</span>
                    <span className="rounded-full border border-white/25 bg-white/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-white">{userProfile.courses.length} active courses</span>
                  </div>
                </div>
              </motion.section>
              ...
              <motion.div
                ...
                id="discover-course-strip"
              >
                <div className="rounded-[1.5rem] bg-white p-5 shadow-sm ring-1 ring-slate-100 sm:p-6">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-fuchsia-600">Your learning map</p>
                      <h3 className="mt-1 text-lg font-semibold text-slate-950">Courses in common start here</h3>
                    </div>
                    <BookOpen className="h-5 w-5 text-fuchsia-500" />
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {userProfile.courses.length > 0 ? userProfile.courses.slice(0, 6).map((course) => (
                      <span key={course} className="rounded-xl bg-[#f4effb] px-3 py-2 text-[10px] font-bold text-[#55406f]">{course.split(':')[0]}</span>
                    )) : <span className="text-xs text-slate-500">Add your courses to unlock more precise matches.</span>}
                  </div>
                </div>
                <div className="rounded-[1.5rem] bg-fuchsia-500 p-5 text-white shadow-sm sm:p-6">
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-fuchsia-100">A better study rhythm</p>
                  <p className="mt-3 text-lg font-semibold leading-snug">Small steps, shared focus, stronger results.</p>
                  <div className="mt-5 h-1.5 overflow-hidden rounded-full bg-white/25"><div className="h-full w-2/3 rounded-full bg-white" /></div>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="flex flex-col gap-2 border-b border-slate-200 pb-4 md:flex-row md:items-center md:justify-between"
                id="discover-header"
              >
          ...
          {activeTab === 'chats' && (
            <div className="space-y-8" id="view-chats">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.5 }}
                className="relative overflow-hidden rounded-[1.75rem] bg-[#28203c] px-6 py-8 text-white shadow-xl shadow-violet-950/10 sm:px-10"
                id="chats-header"
              >
                <div className="absolute -right-16 -top-20 h-56 w-56 rounded-full bg-fuchsia-500/80" />
                <div className="relative max-w-2xl">
                  <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-fuchsia-200">Your study circle</p>
                  <h2 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Connections that keep you moving.</h2>
                  <p className="mt-3 max-w-xl text-sm leading-6 text-violet-100">Message your study buddies, coordinate focus time, and turn good conversations into productive sessions.</p>
                </div>
              </motion.div>
              <div className="-mt-2" id="chats-description">
                <p className="text-xs text-slate-500">Your private study rooms and co-working sessions.</p>
              </div>
              <ChatPanel
          ...
          {activeTab === 'profile' && (
            <div className="space-y-8" id="view-profile">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.2 }}
              ...
              >
                <div className="absolute -right-16 -top-20 h-56 w-56 rounded-full bg-fuchsia-500/80" />
                <div className="relative max-w-2xl">
                  <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-fuchsia-200">Your learning identity</p>
                  <h2 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Shape your study experience.</h2>
                  <p className="mt-3 max-w-xl text-sm leading-6 text-violet-100">Keep your courses, preferences, and availability current so every match feels relevant to the way you learn.</p>
                </div>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.12 }} transition={{ duration: 0.5, delay: 0.08 }}>
                <ProfileForm
                  profile={userProfile}
