...
export default function ProfileForm({ profile, onSaveProfile, message }: ProfileFormProps) {
  ...
  const [university, setUniversity] = useState(profile.university || profile.email.split('@')[1] || '');
  ...
  return (
    <form onSubmit={handleSubmit} id="profile-setup-form" className="space-y-8 rounded-[1.5rem] border border-slate-100 bg-white p-5 shadow-sm sm:p-6 md:p-8">
      <div className="border-b border-slate-100 pb-5" id="profile-form-header">
        <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-fuchsia-50 text-fuchsia-600"><GraduationCap className="w-5 h-5" /></span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="profile-basic-fields">
        ...
        <div className="space-y-1.5" id="field-group-university">
          <label htmlFor="student-university" className="text-xs font-bold uppercase tracking-wider text-slate-900 flex items-center gap-1.5">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-100 bg-white px-3 py-2 text-xs outline-none focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-500/10"
          />
          <span className="mt-1 block text-[10px] text-slate-400">Only students from this university will appear in your discovery list.</span>
        </div>
                ...
                return (
                  <button
                    ...
                    className={`text-left text-[11px] p-3 rounded-xl border transition-all duration-150 flex items-center justify-between ${
                      isSelected
                        ? 'border-fuchsia-500 bg-fuchsia-50 text-[#55406f] font-semibold'
                  ...
                  >
                    ...
                    <span className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center text-[8px] ${
                      isSelected ? 'bg-fuchsia-500 border-fuchsia-500 text-white' : 'border-slate-300 bg-white'
                    }`}>
                      {isSelected && "✓"}
                    </span>
                  </button>
            ...
            return (
              <button
                ...
                className={`py-1.5 px-2 rounded-lg text-[10px] text-center border transition-all duration-150 truncate ${
                  isSelected
                    ? 'border-fuchsia-500 bg-fuchsia-500 text-white font-semibold'
                    : 'border-slate-100 hover:border-slate-200 text-slate-600 bg-white'
                }`}
                title={slot}
              >
      ...
      <div className="pt-4 flex justify-end border-t border-slate-100" id="profile-form-footer">
        <button
          type="submit"
          id="save-profile-btn"
          className="rounded-xl bg-fuchsia-500 px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-white transition-colors duration-150 hover:bg-fuchsia-600"
        >
