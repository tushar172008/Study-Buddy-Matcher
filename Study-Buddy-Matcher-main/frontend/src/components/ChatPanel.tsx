...
export default function ChatPanel({
  ...
  if (matchedBuddies.length === 0) {
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex min-h-[min(500px,60dvh)] flex-col items-center justify-center rounded-[1.5rem] border border-violet-100 bg-white p-5 text-center shadow-sm sm:p-8" id="empty-chat-panel">
        <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-fuchsia-50 text-fuchsia-600 ring-1 ring-fuchsia-100" id="empty-chat-icon">
          <Shield className="w-6 h-6" />
        </div>
        <h4 className="text-sm font-bold uppercase tracking-wider text-[#28203c]" id="empty-chat-title">Your study rooms are ready</h4>
        <p className="mt-2 max-w-xs text-xs leading-relaxed text-slate-500" id="empty-chat-desc">
        ...
        <div className="mt-5 h-1.5 w-24 overflow-hidden rounded-full bg-violet-100"><div className="h-full w-2/3 rounded-full bg-fuchsia-500" /></div>
      </motion.div>
    );
  }

  return (
    <div className="grid h-[min(700px,calc(100dvh-150px))] min-h-[560px] grid-cols-1 grid-rows-[minmax(112px,28%)_1fr] overflow-hidden rounded-[1.5rem] border border-violet-100 bg-white shadow-sm md:h-[600px] md:min-h-0 md:grid-cols-3 md:grid-rows-1" id="chat-panel-container">
      {/* Left Column: Buddy List */}
      <div className="border-b border-slate-100 flex flex-col bg-slate-50/40 md:border-b-0 md:border-r md:col-span-1 min-h-0" id="chat-buddies-list">
      ...
      <div className="md:col-span-2 flex flex-col bg-white h-full min-h-0 relative" id="chat-conversation-area">
        {selectedBuddy && activeSession ? (
          <>
            {/* Conversation Header (Premium Glassmorphic Blur Effect) */}
            <div className="p-3 border-b border-slate-100 flex items-center justify-between gap-2 bg-white/75 backdrop-blur-md sticky top-0 z-20" id="chat-convo-header">
              ...
              <div className="flex items-center gap-1 sm:gap-2 shrink-0" id="chat-header-actions">
                <button
                  type="button"
                  id={`quick-session-btn-${selectedBuddy.id}`}
                  onClick={() => onOpenSchedule(selectedBuddy, true)}
                  className={`relative px-2 py-1.5 sm:px-2.5 rounded-xl text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all active:scale-95 ${
                    selectedBuddy.isCurrentlyFree
