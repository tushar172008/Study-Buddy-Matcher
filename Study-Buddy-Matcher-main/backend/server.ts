...
function publicProfile(user: any) {
  ...
  return {
    ...
    university: profile.university || user.email.split('@')[1] || "Unknown University",
...
app.post("/api/auth/signup", (req, res) => {
  const { email, password, name, university, major, courses, studyStyle, locationPreference, availability, bio } = req.body;
  
  if (!email || !password || !name || !university) {
    return res.status(400).json({ error: "Name, university, email, and password are required fields." });
  }
